...
public sealed class InstallerService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var relativePath in relativePaths)
        {
            ...
            var fullPath = GetSafePath(baseDirectory, relativePath);
            if (File.Exists(fullPath))
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return files.Count(relativePath => File.Exists(GetSafePath(targetDirectory, relativePath)));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var fullPath = GetSafePath(targetDirectory, relativePath);
    ...
    private static string Normalize(string relativePath)
    {
        ...
        if (Path.IsPathRooted(normalized)
            ...
            || normalized.Split(Path.DirectorySeparatorChar).Any(segment => segment is ".." or "."))
        {
            throw new InvalidDataException($"Manifest path is not a safe relative path: {relativePath}");
        }

        return normalized;
    }

    private static string GetSafePath(string baseDirectory, string relativePath)
    {
        var normalizedBase = Path.GetFullPath(baseDirectory)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        var fullPath = Path.GetFullPath(Path.Combine(normalizedBase, Normalize(relativePath)));
        var basePrefix = normalizedBase + Path.DirectorySeparatorChar;

        if (!fullPath.StartsWith(basePrefix, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidDataException($"Manifest path escapes the target directory: {relativePath}");
        }
